...
from albumentations import HorizontalFlip, GridDistortion, OpticalDistortion, ChannelShuffle, CoarseDropout, CenterCrop, \
    Crop, Rotate
...
def augment_data(images, masks, save_path, augment=True):
    ...
    for x, y in tqdm(zip(images, masks), total=len(images)):
        ...
        name = os.path.basename(x).split("/")[-1].split(".")[0]
        # print(os.path.basename(x).split("/")[-1].split(".")[0])
        ...
        for i, m in zip(X, Y):
            ...
            # print(os.path.join(save_path, "image"))
            image_path = os.path.join(save_path, "image", tmp_image_name)
            mask_path = os.path.join(save_path, "mask", tmp_mask_name)
            # print(image_path, mask_path)

            if not cv2.imwrite(image_path, i):
                raise Exception("Could not write image")

            if not cv2.imwrite(mask_path, m):
